...
class CacheManagerComponent extends ComponentAbstract
{
    ...
    {
        add_action('wp_before_admin_bar_render', array($this, 'addCustomMenuItemToAdminBar'));
        add_action('admin_init', array($this, 'clearRfQueryCachePage'));
        add_action('init', [$this, 'deleteExpiredCache']);
        add_action('admin_notices', array($this, 'displayCacheClearedNotice'));
    }
    ...
    {
        if (isset($_GET['clear_rf_query_cache']) && $_GET['clear_rf_query_cache'] == 'true') {
            global $wpdb;
            ...
            $transients = $wpdb->get_col(
                ...
                ",
                    '_transient_timeout_' . $wpdb->esc_like($cache_key_prefix) . '%',
            ...
            $taxonomy_option_prefix = 'realtyna_mls_on_the_fly_taxonomy_after_key_';
            $wpdb->query(
                $wpdb->prepare(
