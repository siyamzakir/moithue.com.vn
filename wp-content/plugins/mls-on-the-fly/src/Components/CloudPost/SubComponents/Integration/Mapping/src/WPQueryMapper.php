...
class WPQueryMapper
{
    ...
    private function addMetaFilters(): void
    {
        ...
        if (isset($this->wp_query->query_vars['meta_key'], $this->wp_query->query_vars['meta_value']) &&
    ...
    private function addRegularFilter(array $condition): void
    {
        ...
        if (($condition['rfMethod'] == 'in' || $condition['rfMethod'] == 'IN') && is_array(
                $condition['rfValue']
    ...
    {
        // Extract taxonomy query information from the WordPress query
        $WPTaxQueries = $this->wp_query->tax_query->queries ?? [];
        $queries = [];
        $WPTaxQueries = $this->extractTaxQueryInfo($WPTaxQueries);
        // Process each taxonomy query
        foreach ($WPTaxQueries as $WPTaxQuery) {
            $taxonomy = $WPTaxQuery['taxonomy'];
            if (str_contains($taxonomy, '|')) {
                $taxonomy = explode('|', $taxonomy)[0];
            }
            foreach ($WPTaxQuery['terms'] as $term) {
                // Retrieve the term object by specified field and term identifier
                $WPTerm = get_term_by($WPTaxQuery['field'], $term, $taxonomy);
                if ($WPTerm) {
                    // Map the term to an RFQuery field
                    ...
                    // Apply any necessary name replacements
                    if ($replaces) {
                        foreach ($replaces as $replace) {
                            if ($WPTerm->name == $replace['replace']) {
                                $WPTerm->name = $replace['search'];
                            }
                        }
                    }

                    // Handle hierarchical relationships if applicable
                    if ($RFChildField && $WPTerm->parent) {
                        $ParentWPTerm = get_term_by('id', $WPTerm->parent, $taxonomy);
                        if ($replaces) {
                            foreach ($replaces as $replace) {
                                if ($ParentWPTerm->name == $replace['replace']) {
                                    $ParentWPTerm->name = $replace['search'];
                                }
                            }
                        }

                        // Group terms under parent-child relationships
                        $queries[$RFField][] = $ParentWPTerm->name;
                        $queries[$RFChildField][] = $WPTerm->name;
                    } else {
                        // Simply map terms under their respective fields
                        if (is_array($RFField)) {
                            $queries['open'] = 'OR';
                            foreach ($RFField as $SingleRFField) {
                                $queries[$SingleRFField][] = $WPTerm->name;
                            }
                            $queries['close'] = 'OR';
                        } else {
                            $queries[$RFField][] = $WPTerm->name;
                        }
                    }
                    ...
                    if (is_array($RFField)) {
                        $queries['open'] = 'OR';
                        foreach ($RFField as $SingleRFField) {
                            $queries[$SingleRFField][] = $term;
                        }
                        $queries['close'] = 'OR';
                    } else {
                        $queries[$RFField][] = $term;
                    }
                }
            }
        }

        // Apply filters to the RFQuery object based on collected term data
        foreach ($queries as $RFField => $RFValues) {
            ...
            $RFValues = array_unique($RFValues);  // Eliminate duplicate values
            $count = count($RFValues);

            // Determine the filter method based on the number of values
            if ($count > 0) {
                if ($count > 1) {
                    $this->RFQuery->add_filter(
                        method: 'in',
