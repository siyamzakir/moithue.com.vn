    ...
    protected function parseVariables(string $value, string $returnType = 'string')
    {
        ...
        foreach ($matches[1] as $match) {
            ...
            if (property_exists(
                    $this->RFProperty,
                    $match
                ) && isset($this->RFProperty->$match) && $this->RFProperty->$match) {
                if ($returnType == 'boolean' && is_array($this->RFProperty->$match)) {
                    $value = str_replace('{' . $match . '}', strval(true), $value);
                } elseif ($returnType == 'array') {
                    $value = $this->RFProperty->$match;
                } elseif ($returnType == 'stringed-array') {
                    $items = [];
                    if (is_array($this->RFProperty->$match)) {
                        foreach ($this->RFProperty->$match as $item) {
                            $items[] = "'$item'";
                        }
                    } else {
                        $items[] = "'" . $this->RFProperty->$match . "'";
                    }
                    $value = str_replace('{' . $match . '}', '[' . implode(',', $items) . ']', $value);
                } elseif ($returnType != 'boolean') {
                    if (is_array($this->RFProperty->$match)) {
                        $this->RFProperty->$match = implode(',', $this->RFProperty->$match);
                    }
                    $value = str_replace('{' . $match . '}', strval($this->RFProperty->$match), $value);
                }
            } else {
